...
RegisterAccessorModel::RegisterAccessorModel(
    ...
    : _abstractAccessor(abstractAccessor), _channelNumber(0),
      _cookedHexColumnIndex(-1), _rawColumnIndex(-1), _rawHexColumnIndex(-1),
      _nColumns(1) {
  ...
  if (_abstractAccessor->isIntegral() &&
      !_abstractAccessor->rawDataType()
           .isIntegral()) { // there will be no raw hex column
...
int RegisterAccessorModel::columnCount(
    const QModelIndex & /*modelIndex*/) const {
  return _nColumns;
}

QVariant RegisterAccessorModel::data(const QModelIndex &modelIndex,
                                     int role) const {
  switch (role) {
  case Qt::DisplayRole:
  case Qt::EditRole:
    if (modelIndex.row() >=
        PreferencesProviderSingleton::Instance().getValue<int>("maxWords")) {
  ...
  case Qt::BackgroundRole:
    if (_isModified[_channelNumber]
                   [modelIndex.row()]) { // change background color for modified
                                         // rows
...
Qt::ItemFlags
RegisterAccessorModel::flags(const QModelIndex &modelIndex) const {
  return Qt::ItemIsEditable | QAbstractTableModel::flags(modelIndex);
}

bool RegisterAccessorModel::setData(const QModelIndex &modelIndex,
                                    const QVariant &value, int role) {
  if (role == Qt::EditRole) {
    bool settingSuccessful;
    if ((modelIndex.column() == _rawColumnIndex) ||
        (modelIndex.column() == _rawHexColumnIndex)) {
      settingSuccessful = _abstractAccessor->setRawData(
          _channelNumber, modelIndex.row(), value);
    } else {
      settingSuccessful =
          _abstractAccessor->setData(_channelNumber, modelIndex.row(), value);
    }
...
QVariant RegisterAccessorModel::headerData(int section,
  ...
  if (role == Qt::DisplayRole) {
    ...
    if (orientation == Qt::Horizontal) {
      if (section == 0) {
        if (_cookedHexColumnIndex !=
            -1) { // there is a cooked hex column, so add (dec)
      ...
      if (section == _rawColumnIndex) {
        if (_rawHexColumnIndex !=
            -1) { // there is a raw hex column, so add (dec)
