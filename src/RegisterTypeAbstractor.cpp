...
createTypedAccessor(ChimeraTK::RegisterInfo const &registerInfo,
  ...
  auto nElements =
      std::min(PreferencesProviderSingleton::Instance().getValue<unsigned int>(
                   "maxWords") +
                   1,
  ...
  case ChimeraTK::DataType::none: {
    ...
    auto accessor = device.getTwoDRegisterAccessor<USER_DATA_TYPE>(
        registerInfo.getRegisterName(), nElements);
    return std::make_shared<RegisterTypeAbstractorImpl<USER_DATA_TYPE>>(
  ...
  default: {
    ...
    auto rawAccessorCreatorLambda = [&](auto arg) {
      auto accessor = device.getTwoDRegisterAccessor<decltype(arg)>(
          registerInfo.getRegisterName(), nElements, 0,
          {ChimeraTK::AccessMode::raw});
      returnValue = std::make_shared<
          RegisterTypeAbstractorRawImpl<decltype(arg), USER_DATA_TYPE>>(
          accessor, dataDescriptor.rawDataType());
    };
