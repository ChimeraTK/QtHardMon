...
: QStyledItemDelegate(parent_), _doubleSpinBoxPrecision(DOUBLE_SPINBOX_DEFAULT_PRECISION) {}
 ...
 */
QString CustomDelegates::displayText(const QVariant& value, const QLocale& locale) const {
...
void CustomDelegates::setEditorData(QWidget* editor, const QModelIndex& index) const {
...
void CustomDelegates::setModelData(QWidget* editor, QAbstractItemModel* model, const QModelIndex& index) const {
  if(index.data(Qt::EditRole).type() == HexDataType) {
    HexSpinBox* spinBox = static_cast<HexSpinBox*>(editor);
...
void CustomDelegates::setDoubleSpinBoxPrecision(unsigned int doubleSpinBoxPrecision) {
