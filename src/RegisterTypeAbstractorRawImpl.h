...
/** This is a double templated accessor abstractor with raw and cooked data type.
 *  It is based upon the single-typed RegisterTypeAbstractorImpl with the raw type
 *  as user type. For those functions which do cooked values, it does the corresponding
 *  conversion.
 */
...
class RegisterTypeAbstractorRawImpl: public RegisterTypeAbstractorImpl<RAW_DATA_TYPE>{
 public:
  ...
  /// Override because it returns cooked data while the accessor under the hood is raw.
  ...
  /// Override because it expects cooked data while the accessor under the hood is raw.
  ...
  /// Override because this is for the cooked type while the base implementation would return
  /// for the raw type.
...
template <class RAW_DATA_TYPE, class COOKED_DATA_TYPE>
RegisterTypeAbstractorRawImpl<RAW_DATA_TYPE, COOKED_DATA_TYPE>::RegisterTypeAbstractorRawImpl(ChimeraTK::TwoDRegisterAccessor<RAW_DATA_TYPE> const & accessor, ChimeraTK::DataType const & rawDataType) : RegisterTypeAbstractorImpl<RAW_DATA_TYPE>(accessor, rawDataType) , _cachedCookedDataValid(false){
...
template <class RAW_DATA_TYPE, class COOKED_DATA_TYPE>
QVariant RegisterTypeAbstractorRawImpl<RAW_DATA_TYPE, COOKED_DATA_TYPE>::data(unsigned int channelIndex, unsigned int elementIndex) const{
...
// Get the data as cooked and put it into the HexData user type. Return this as QVariant.
// Notice: For QString cooked dataq the correct return value would be an empty variant, which
...
template <class RAW_DATA_TYPE, class COOKED_DATA_TYPE>
QVariant RegisterTypeAbstractorRawImpl<RAW_DATA_TYPE, COOKED_DATA_TYPE>::dataAsHex(unsigned int channelIndex, unsigned int elementIndex) const{
...
template <class RAW_DATA_TYPE, class COOKED_DATA_TYPE>
QVariant RegisterTypeAbstractorRawImpl<RAW_DATA_TYPE, COOKED_DATA_TYPE>::rawData(unsigned int channelIndex, unsigned int elementIndex) const{
...
template <class RAW_DATA_TYPE, class COOKED_DATA_TYPE>
QVariant RegisterTypeAbstractorRawImpl<RAW_DATA_TYPE, COOKED_DATA_TYPE>::rawDataAsHex(unsigned int channelIndex, unsigned int elementIndex) const{
  QVariant returnValue;
...
template<class RAW_DATA_TYPE, class COOKED_DATA_TYPE>
bool RegisterTypeAbstractorRawImpl<RAW_DATA_TYPE, COOKED_DATA_TYPE>::setData(unsigned int channelIndex, unsigned int elementIndex, const QVariant & data){
...
template<class RAW_DATA_TYPE, class COOKED_DATA_TYPE>
bool RegisterTypeAbstractorRawImpl<RAW_DATA_TYPE, COOKED_DATA_TYPE>::setRawData(unsigned int channelIndex, unsigned int elementIndex, const QVariant & data){
