...
public:
  ...
  std::map<std::string, double> const &getDoubleValues() const {
...
private:
  ...
  void getMap(std::map<std::string, int> const *&map) const {
    map = &_intValues;
  }
  void getMap(std::map<std::string, double> const *&map) const {
    map = &_doubleValues;
  };
  void getMap(std::map<std::string, std::string> const *&map) const {
    map = &_stringValues;
  }
...
template <class T>
T ConfigFileReaderWriter::getValue(std::string const &variableName,
  ...
  typename std::map<std::string, T>::const_iterator valueIter =
...
template <class T>
void ConfigFileReaderWriter::setValue(std::string const &variableName,
                                      T const &value) {
  // get the map using the getMap functions, which are overloaded by type
