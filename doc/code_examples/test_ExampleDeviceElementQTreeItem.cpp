...
struct ExampleDeviceElementQTreeItem_fixtureBase {
  ...
  DeviceElementQTreeItem_fixtureBase() {
    ...
    propertiesWidgetProvider.registerWidget(ExampleDeviceElementQTreeItem::DataType, propertiesWidget, -1);
  }
};
...
struct ExampleDeviceElementQTreeItem_fixture : public ExampleDeviceElementQTreeItem_fixtureBase,
  ...
  : DeviceAccessSetup_fixture(dmapFile, deviceName) {
    ...
    item = new ExampleDeviceElementQTreeItem(device, registerCatalogue.getRegister(mtca4u::RegisterPath(registerPath)),
        treeWidget, propertiesWidgetProvider);
  }
};

BOOST_AUTO_TEST_CASE(ExampleDeviceElementQTreeItem_constructor) {
  ExampleDeviceElementQTreeItem_fixture fixture("test_files/test_dmap.dmap", "DEVICE_NAME", "MODULE/REGISTER");

  // Check if the type returned is appropriate, so it can be recognized by
  // PropertiesWidgetProvider
  BOOST_CHECK_EQUAL(fixture.item->type(), static_cast<int>(ExampleDeviceElementQTreeItem::DataType));
  ...
  BOOST_CHECK_EQUAL(fixture.treeWidget->topLevelItem(0)->text(0).toStdString(), "MODULE");
  BOOST_CHECK_EQUAL(fixture.treeWidget->topLevelItem(0)->childCount(), 1);
  BOOST_CHECK_EQUAL(fixture.treeWidget->topLevelItem(0)->child(0)->text(0).toStdString(), "REGISTER");
}
...
BOOST_AUTO_TEST_CASE(ExampleDeviceElementQTreeItem_fillsRegisterProperties) {
  ...
  BOOST_CHECK_EQUAL(fixture.propertiesWidget->ui->registerNameDisplay->text().toStdString().c_str(), "");
  BOOST_CHECK_EQUAL(fixture.propertiesWidget->ui->moduleDisplay->text().toStdString().c_str(), "");
  BOOST_CHECK_EQUAL(fixture.propertiesWidget->ui->registerNElementsDisplay->text().toStdString().c_str(), "");

  fixture.item->updateRegisterProperties();

  // Check if updateRegisterProperties() sets ExamplePropertiesWidget to
  // appropriate state
  BOOST_CHECK_EQUAL(fixture.propertiesWidget->ui->registerNameDisplay->text().toStdString().c_str(), "REGISTER");
  BOOST_CHECK_EQUAL(fixture.propertiesWidget->ui->moduleDisplay->text().toStdString().c_str(), "MODULE");
  BOOST_CHECK_EQUAL(fixture.propertiesWidget->ui->registerNElementsDisplay->text().toStdString().c_str(), "1");
}
