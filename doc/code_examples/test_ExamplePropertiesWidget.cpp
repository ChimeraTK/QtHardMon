...
BOOST_AUTO_TEST_CASE(ExamplePropertiesWidget_QWidget) {
  ...
  bool qWidgetIsBaseOfRegisterTreeWidget =
      std::is_base_of<QWidget, ExamplePropertiesWidget>::value;
...
BOOST_AUTO_TEST_CASE(NumericAddressedRegisterQTreeItem_constructor) {
  ...
  BOOST_CHECK_EQUAL(
      fixture.widget->ui->registerNameDisplay->text().toStdString().c_str(),
      "");
  BOOST_CHECK_EQUAL(
      fixture.widget->ui->moduleDisplay->text().toStdString().c_str(), "");
  BOOST_CHECK_EQUAL(fixture.widget->ui->registerNElementsDisplay->text()
                        .toStdString()
                        .c_str(),
                    "");
