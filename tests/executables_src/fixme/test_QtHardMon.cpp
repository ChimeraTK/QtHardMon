...
BOOST_AUTO_TEST_CASE(QtHardMon_defaultSettings) {
  ...
  BOOST_CHECK_EQUAL(
      preferencesProvider.getValue<int>("floatPrecision"), CustomDelegates::DOUBLE_SPINBOX_DEFAULT_PRECISION);
...
struct QtHardMon_populatesDeviceList_fixture : public QtHardmon_fixtureBase {
  ...
  QtHardMon_populatesDeviceList_fixture(const std::string& DmapFile) : dmapFile(DmapFile) {
...
BOOST_AUTO_TEST_CASE(QtHardMon_properlyCatchesInvalidDeviceSelected) {
  ...
  QList<QListWidgetItem*> items = fixture.qtHardMon->ui.deviceListWidget->findItems("INVALID", Qt::MatchExactly);
  if(items.size() > 0) {
    BOOST_CHECK_NO_THROW(fixture.qtHardMon->ui.deviceListWidget->setCurrentItem(items.at(0)));
  }
  else {
...
struct QtHardmon_populatesRegisterTree_fixture : public QtHardMon_populatesDeviceList_fixture {
  ...
  void switchDeviceSelection(const std::string& DeviceNameToSelect) {
    QList<QListWidgetItem*> items =
        qtHardMon->ui.deviceListWidget->findItems(DeviceNameToSelect.c_str(), Qt::MatchExactly);
    if(items.size() > 0) {
...
struct QtHardmon_populatesRegisterProperties_fixture : public QtHardmon_populatesRegisterTree_fixture {
  ...
  QtHardmon_populatesRegisterProperties_fixture(
      const std::string& DmapFile, const std::string& DeviceNameToSelect, std::vector<std::string> RegisterToSelect)
...
BOOST_AUTO_TEST_CASE(QtHardMon_populatesRegisterProperties) {
  ...
  TestUtilities::checkRegisterProperties(
      fixture.qtHardMon->ui.registerPropertiesWidget, "MODULE1", "APP0", "1", "32", "2", "8", "32", "0", "1");
}
...
BOOST_AUTO_TEST_CASE(QtHardMon_autoselectsRegister) {
  ...
  TestUtilities::checkRegisterProperties(
      fixture.qtHardMon->ui.registerPropertiesWidget, "WORD_USER1", "MODULE1", "1", "32", "1", "4", "16", "3", "1");
...
BOOST_AUTO_TEST_CASE(QtHardMon_populatesDataTable) {
  ...
  TestUtilities::checkTableData(
      fixture.qtHardMon->ui.registerPropertiesWidget, {std::make_tuple(0, 0, 0.0), std::make_tuple(0, 0, 0.0)});
}
...
BOOST_AUTO_TEST_CASE(QtHardMon_writesRegister) {
  ...
  TestUtilities::checkTableData(
      fixture.qtHardMon->ui.registerPropertiesWidget, {std::make_tuple(10, 10, 10.0), std::make_tuple(0, 0, 0.0)});
}
...
BOOST_AUTO_TEST_CASE(QtHardMon_populatesSequenceRegisterProperties) {
  QtHardmon_populatesRegisterProperties_fixture fixture("test_QtHardMon_valid_dummy.dmap", "NUMDEV_MULT",
      {"APP0", "AREA_MULTIPLEXED_SEQUENCE_DAQ0_ADCA", "SEQUENCE_DAQ0_ADCA_10"});

  TestUtilities::checkRegisterProperties(fixture.qtHardMon->ui.registerPropertiesWidget, "SEQUENCE_DAQ0_ADCA_10",
