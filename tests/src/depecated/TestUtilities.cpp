...
void TestUtilities::checkRegisterProperties(RegisterPropertiesWidget* widget, const std::string& registerName,
...
void TestUtilities::checkModuleProperties(
    ModulePropertiesWidget* widget, const std::string& moduleName, const std::string& registerNElements) {
  BOOST_CHECK_EQUAL(widget->ui->moduleDisplay->text().toStdString().c_str(), moduleName);
  BOOST_CHECK_EQUAL(widget->ui->registerNElementsDisplay->text().toStdString().c_str(), registerNElements);
}

void TestUtilities::checkRegisterProperties(GenericRegisterPropertiesWidget* widget, const std::string& registerName,
...
void TestUtilities::checkTableData(
    RegisterPropertiesWidget* widget, std::vector<std::tuple<int, int, double>> tableDataValues, int size) {
  if(size == 0) {
    ...
    for(uint i = 0; i < tableDataValues.size(); ++i) {
      ...
      BOOST_CHECK_EQUAL(
          widget->ui->valuesTableWidget->item(i, 2)->text().toDouble(), std::get<2>(tableDataValues.at(i)));
    }
  }
  else {
    ...
    for(int i = 0; i < size; ++i) {
      ...
      //    1)->text().toInt(), std::get<1>(tableDataValues.at(i)));
      BOOST_CHECK_EQUAL(
          widget->ui->valuesTableWidget->item(i, 2)->text().toDouble(), std::get<2>(tableDataValues.at(0)));
    }
  }
}
...
void TestUtilities::setTableValue(
...
void TestUtilities::checkTableData(
    GenericRegisterPropertiesWidget* widget, std::vector<std::tuple<int, int, double>> tableDataValues, int size) {
  if(size == 0) {
